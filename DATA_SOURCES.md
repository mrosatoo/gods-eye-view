...
| **IMF PortWatch** | Dated chokepoint transit activity (daily vessel transit counts at Hormuz, Suez, Bab el-Mandeb, Malacca, Singapore, Cape) | [IMF Terms of Use](https://www.imf.org/external/terms.htm) — public research data; see PortWatch portal terms | "IMF PortWatch" (dated daily activity — not live congestion) |
...
- **IMF PortWatch.** `/api/portwatch` proxies dated daily transit-count data from IMF PortWatch's public portal (ArcGIS Feature Service). Data is cached server-side for 60 minutes. PortWatch updates daily with a 1–3 day observation lag; the overlay labels each reading with its explicit observation date and never claims "live congestion" or "queue time". When the upstream is unreachable or source admission is pending, the proxy returns `error: "source_unavailable"` and the overlay shows "Source admission pending" — it never fabricates transit counts. Four thesis chokepoints (Hormuz, Suez, Bab el-Mandeb, Malacca) render as context cards with trade-corridor annotations; secondary chokepoints render standard rectangle overlays.
