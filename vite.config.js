...
const OVERPASS_UPSTREAMS = [
  ...
  // Plain-HTTP twins: some egress paths abort TLS to the public Overpass hosts
  // with unexpected-EOF while HTTP answers the same QL. Keep each host's HTTPS
  // attempt first, then its HTTP twin, and promote lz4 HTTP ahead of mirrors
  // that hang the full OVERPASS_TIMEOUT_MS on TLS so traffic roads can settle
  // without a 20s+ stall (fielded 2026-09-11 — TomTom live cov stuck at 0%/
  // syncing when every HTTPS upstream timed out before HTTP was reached).
 ...
 * (HTTP twin tried automatically when TLS to celestrak.org fails).
...
function celestrakProxy() {
  ...
  async function fetchUpstream(group) {
    // Prefer HTTPS; fall back to plain HTTP when TLS is broken on the egress
    // path (fielded 2026-09-11: https://celestrak.org aborts with unexpected
    // EOF while http://celestrak.org serves all GP groups). Same GROUP/FORMAT
    // contract either way — the /api/celestrak browser surface is unchanged.
    const origins = [
      'https://celestrak.org/NORAD/elements/gp.php',
      'http://celestrak.org/NORAD/elements/gp.php',
    ];
    const headers = {
    ...
    throw lastError || new Error('celestrak upstream failed');
  }
      ...
      server.middlewares.use('/api/opensky', async (req, res) => {
        try {
          ...
          if (inConnectCooldown) {
            if (await serveAdsbLolPointFallback(req, res, requestedMode, 'opensky_unreachable_adsblol_primary')) return;
            res.writeHead(502, buildOpenSkyHeaders({
              cacheStatus: 'MISS',
              requestedMode,
              usedMode: 'none',
              reason: 'opensky_unreachable',
            }));
            res.end(JSON.stringify({ error: 'OpenSky unreachable (TLS/network); no regional view available.' }));
            return;
          }
          ...
          const connectCooldownMs = Math.min(60_000 * Math.pow(3, Math.min(_openskyConnectFailCount - 1, 4)), 1_800_000);
          _openskyConnectCooldownUntil = Date.now() + connectCooldownMs;
          console.error('[OpenSky Proxy] Connectivity failure #%d, cooldown %ds: %s', _openskyConnectFailCount, Math.round(connectCooldownMs / 1000), e.message);
...
function keySetupEndpoint() {
  const respond = (res, statusCode, payload) => {
    ...
    // INTENTIONAL: keep DENY even when GEV_FRAME_ANCESTORS allows the desk to
    // frame the app document. Credential routes stay unframeable.
    res.setHeader('Cache-Control', 'no-store');
...
function portWatchProxy() {
  const ADMITTED_SOURCES = new Set();
  ...
  const VALID_TRANSIT_UNITS = new Set(['transits/day', 'n_trips']);
  const VALID_CHOKEPOINTS = new Set(Object.keys(PORTWATCH_REGION_IDS));

  function validateObservationDate(dateStr) {
  ...
  function validateTransitCount(value) {
  ...
  return {
    ...
    configureServer(server) {
      server.middlewares.use('/api/portwatch', async (req, res) => {
        ...
        if (!VALID_CHOKEPOINTS.has(chokepoint)) {
          res.writeHead(400, { 'Content-Type': 'application/json' });
          res.end(JSON.stringify({ error: 'invalid_chokepoint' }));
          return;
        }

        if (!ADMITTED_SOURCES.has('imf-portwatch')) {
          const result = {
            chokepoint,
            ...
            admissionNote: 'IMF PortWatch source not admitted. Dataset ID, region mapping, transit-unit schema, and intended-use terms require admission per Joint Spec §4.2.',
          };
...
export default defineConfig(({ mode }) => {
  ...
  // OSATO LOCAL PATCH (desk embed): allow framing only from explicit parent
  // origins via GEV_FRAME_ANCESTORS. Empty => upstream DENY / 'none' behavior.
  // See /workspace/gods-eye-view/PATCH-OSATO-DESK.md — not upstream.
  const frameParents = String(env.GEV_FRAME_ANCESTORS || '')
    .split(/[\s,]+/)
    .filter(Boolean)
    .filter((candidate) => {
      try {
  ...
  const frameHeaders = frameParents.length
    // X-Frame-Options deliberately omitted: modern browsers have no working
    // Allow-From, and DENY would override the CSP allowlist.
    ? { 'Content-Security-Policy': `frame-ancestors 'self' ${frameParents.join(' ')}` }
  ...
  return {
    ...
    server: {
      ...
      // Framing: default DENY/'none'. When GEV_FRAME_ANCESTORS lists desk
      // origins, CSP allowlist replaces blanket DENY (Osato desk /god-eye iframe).
      // keySetupEndpoint still always sends DENY on credential routes.
